...
export default function CASignUpPage() {
  ...
  const [icaiMembership, setIcaiMembership] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/signup/ca', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fullName, email, password, icaiMembership, practiceName, specialization, locale }),
      });
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${origin}/api/auth/callback?next=/${locale}/ca/clients`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          },
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div className="space-y-3 pt-6">
              ...
              <p className="text-[14px] text-neutral-400 leading-relaxed max-w-sm">
                Join our network of Chartered Accountants and grow your practice. Manage clients effectively with our compliance suite.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:w-1/2 p-8 sm:p-12 flex flex-col justify-between overflow-y-auto max-h-screen lg:max-h-[800px]">
          ...
          <div className="w-full max-w-sm mx-auto space-y-5 my-auto py-8">
            <div className="space-y-1">
              <h1 className="text-2xl font-bold text-neutral-900">{t('caSignUpTitle')}</h1>
              <p className="text-[13px] text-neutral-500">{t('caSignUpSubtitle')}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5 text-left">
              ...
              <div className="space-y-1">
                <label className="text-[12px] font-semibold text-neutral-700 block" htmlFor="icaiMembership">
                  {t('icaiMembershipLabel')}
                </label>
                <input
                  id="icaiMembership"
                  type="text"
                  required
                  value={icaiMembership}
                  onChange={(e) => setIcaiMembership(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-neutral-200 text-neutral-900 placeholder:text-neutral-400 text-[14px] focus:outline-none focus:border-[#ef4d23] focus:ring-1 focus:ring-[#ef4d23]"
                />
              </div>

              <div className="space-y-1">
                <label className="text-[12px] font-semibold text-neutral-700 block" htmlFor="practiceName">
                  {t('practiceNameLabel')}
                </label>
                <input
                  id="practiceName"
              ...
              <div className="space-y-1">
                <label className="text-[12px] font-semibold text-neutral-700 block" htmlFor="specialization">
                ...
                >
                  <option value="Direct Tax">{t('specDirectTax')}</option>
                  <option value="Indirect Tax">{t('specIndirectTax')}</option>
                  <option value="Audit">{t('specAudit')}</option>
                  <option value="Company Law">{t('specCompanyLaw')}</option>
